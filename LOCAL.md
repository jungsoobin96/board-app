...
| v0.4 | 2026-05-26 | woosung.ahn@bespinglobal.com | Issue #10 — frontend 골격 실 도입 (Vite + React + Tailwind + Router). `.env.{dev,stg,prod}.example`에 `VITE_API_URL` + `VITE_APP_TITLE` 추가 (12 §6 정합). §3 frontend dev/preview 명령은 이미 명시되어 있어 본 PR로 *실 발현*. §5.7 Tailwind 트러블슈팅 항목 이미 활용 가능. **사용자 PowerShell `pnpm install` 후 lock commit 필수**. |
